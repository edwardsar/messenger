...
{
    string serverIP;
    cout << "Enter server IP (example: 192.168.44.30): ";
    cin >> serverIP;
...
void registerUser(string &uname, struct sockaddr_in &serveraddr, SOCKET &serverSocket, int serveraddrSize)
{
    ...
    strcpy(cmdBuffer, "reg "); //following build the command to send to the server
    strncat(cmdBuffer, uname.c_str(), cmdBufferSize - sizeof("reg "));
...
void sendMessage( struct sockaddr_in &serveraddr, SOCKET &serverSocket, int serveraddrSize, string &uname)
{
    ...
    strncat(cmdBuffer, recipientUsername, usernameBufferSize);
    ...
    if( strcmp(serverResponseBuffer, "NF") != 0){
        ...
        strcpy(messageBuffer, uname.c_str());
        strncat(messageBuffer, ": ", 2);
        strncat(messageBuffer, mbuf, 100);
